...
export default function CategoryManager({ categories, onSuccess, onShowToast }: CategoryManagerProps) {
  ...
  return (
    <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm p-6 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700">
      ...
      {isOpen && (
        <form onSubmit={handleSubmit} className="mb-6 p-5 bg-gradient-to-br from-gray-50 to-blue-50 dark:from-gray-700 dark:to-blue-900/30 rounded-xl border border-gray-200 dark:border-gray-600 animate-slide-up">
      ...
      <div className="space-y-4">
        <h3 className="font-semibold text-sm text-gray-700 dark:text-gray-300 uppercase tracking-wide mb-3">Expense Categories</h3>
        {categories.filter(c => c.type === 'expense').map((category) => (
          <div key={category.id} className="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-700 rounded-xl hover:bg-gradient-to-r hover:from-gray-50 hover:to-red-50 dark:hover:from-gray-700 dark:hover:to-red-900/30 transition-all duration-200 hover:shadow-md">
            <div>
        ...
        <h3 className="font-semibold text-sm text-gray-700 dark:text-gray-300 uppercase tracking-wide mb-3 mt-6">Income Categories</h3>
        {categories.filter(c => c.type === 'income').map((category) => (
          <div key={category.id} className="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-700 rounded-xl hover:bg-gradient-to-r hover:from-gray-50 hover:to-green-50 dark:hover:from-gray-700 dark:hover:to-green-900/30 transition-all duration-200 hover:shadow-md">
            <span className="font-medium dark:text-gray-200">{category.name}</span>
            <button
              onClick={() => handleDelete(category.id, category.name)}
              className="text-red-600 dark:text-red-400 hover:text-white hover:bg-red-600 dark:hover:bg-red-700 px-3 py-1.5 rounded-lg text-sm font-medium transition-all duration-200"
