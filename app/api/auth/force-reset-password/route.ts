...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!user.force_password_reset) {
      return NextResponse.json({ error: "Password reset not required for this user" }, { status: 400 });
    }
    ...
    // Update password and clear the force reset flag
    ...
    console.error("Error with forced password reset:", error);
